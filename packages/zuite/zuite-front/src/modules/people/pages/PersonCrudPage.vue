...
<template>
  <crud :entity="PersonCrud.instance">
    ...
    <template v-slot:item.hobbies="{value}"><v-chip v-if="value.length > 0" v-for="v in value">{{v}}</v-chip><span v-else></span></template>

    <template v-slot:item.address="{value}">{{value.street}}</template>
