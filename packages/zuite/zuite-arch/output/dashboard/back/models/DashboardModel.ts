...
const DashboardSchema = new mongoose.Schema<IDashboard>({
            ...
            cardVariant: {type: String,  enum: ['text', 'flat', 'elevated', 'tonal', 'outlined', 'plain'], required: true, index: false, unique: false } 
            },
            groupBy: {
            fields: [{type: String,   required: true, index: false, unique: false }],
            dateFormat: {type: String,  enum: ['year', 'month', 'day', 'hour', 'minute', 'second'], required: false, index: false, unique: false },
            render: {type: String,  enum: ['table', 'gallery', 'pie', 'bars'], required: false, index: false, unique: false } 
            },
            paginate: {
            columns: [{type: String,   required: true, index: false, unique: false }],
            orderBy: {type: String,   required: false, index: false, unique: false },
            order: {type: String,  enum: ['asc', 'desc'], required: false, index: false, unique: false } 
            } 
            }]
}, {timestamps: true});
