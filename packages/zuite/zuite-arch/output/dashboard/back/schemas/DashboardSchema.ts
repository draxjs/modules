...
const DashboardBaseSchema = z.object({
    ...
    cards: z.array(
z.object({    entity: z.string().min(1,'validation.required'),
...
z.object({    field: z.string().min(1,'validation.required'),
    ...
    layout: z.object({    cols: z.number().min(0,'validation.required').default(12),
    ...
    groupBy: z.object({    fields: z.array(z.string()),
    dateFormat: z.enum(['year', 'month', 'day', 'hour', 'minute', 'second']).optional().default('day'),
    render: z.enum(['table', 'gallery', 'pie', 'bars']).optional().default('day')}),
    paginate: z.object({    columns: z.array(z.string()).default([]),
